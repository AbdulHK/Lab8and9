...
until [ "$selection" = "0" ]; do
	...
	case $selection in
		...
		3  ) 	curl -s -X GET -H 'Accept: application/json' http://localhost:8080/containers?state=running | python -mjson.tool
			press_enter
			;;
		4  ) 	curl -s -X GET -H 'Accept: application/json' http://localhost:8080/containers/e3bace849c64 | python -mjson.tool
			press_enter
			;;
		5  ) 	curl -s -X DELETE -H 'Accept: application/json' http://localhost:8080/containers/e3bace849c64 | python -mjson.tool
			press_enter
			;;
		...
		7  ) 	curl -X POST -H 'Content-Type: application/json' http://localhost:8080/containers -d '{"image": "49b7d316bf66"}' | python -mjson.tool
			press_enter
			;;
		8  )	curl -X PATCH -H 'Content-Type: application/json' http://localhost:8080/containers/0e6e33c6e20d -d '{"state": "running"}'
			press_enter
			;;
		9  ) 	curl -X PATCH -H 'Content-Type: application/json' http://localhost:8080/containers/0e6e33c6e20d -d '{"state": "stopped"}'
			press_enter
			;;
